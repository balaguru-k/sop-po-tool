...
const Pochecker = (props) => {
  ...
  const TicketReject = async () => {
    try {
      const apiUrl = `${BaseUrl}api/ticket/rejected-tickets?stage=Po_checker&ticketType=${localStorage.getItem("selectedTicketTab") === "Brand" ? "Brand" : "NonBrand"}`;
  ...
  const DataTicket = async () => {
    try {
      const apiUrl = BaseUrl + `api/ticket/getAllByCommonStage/${role}?ticketType=${localStorage.getItem("selectedTicketTab") === "Brand" ? "Brand" : "NonBrand"}`;
      const response = await axios.get(apiUrl, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
        },
  ...
  useEffect(() => {
    ...
    const stompClient = new Client({
      ...
      onConnect: () => {
        ...
        stompClient.subscribe("/topic/Po_checker", (message) => {
  ...
  const DataTickethistab = async () => {
    try {
      const apiUrl = BaseUrl + `api/ticket/getAllComplticketsByStage/${role}?ticketType=${localStorage.getItem("selectedTicketTab") === "Brand" ? "Brand" : "NonBrand"}`;
      const response = await axios.get(apiUrl, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
        },
  ...
  const title_ticket_table = [
    ...
    },
    {
      ...
      render: (updatedAt, _, index) => {
        const timeOnly = updatedAt ? new Date(updatedAt).toLocaleTimeString() : "";
        return (
  ...
  const handleExportClick = async () => {
    if (handleValidation()) {
      try {
        ...
        toast.error("No tickets found for the selected date range.", { duration: 2000 }); // Display a success message
  ...
  const handleNotificationClick = async (notification) => {
    ...
    // Optionally, you can also show the ticket details/modal when clicked
    showModal();
    setMode("checker");
    setIsView(true);
  ...
  const handleMttpUpdate = async (data) => {
    try {
      const token = localStorage.getItem("accessToken");
      const apiUrl = BaseUrl + `mttp-ticket/approver-ticket/${data.id}?status=${data.approveStatus}&remarks=${encodeURIComponent(data.remarks)}`;
      ...
      if (response.status === 200) {
        setSuccessMessage(`Ticket ${data.approveStatus.toLowerCase()} successfully`);
        setTimeout(() => {
          setSuccessMessage(null);
        }, 3000);
        fetchMttpData();
            ...
            <div className="vendor_details_part">
              ...
              <Row>
                ...
                <Col
                  ...
                  className="form_margin validation-error"
                >
                  {ticketdataedit.poApprover ? (
                    <>
                      <Form.Label className="txt_lable form-label">
                        Business Approver
                      </Form.Label>
                      <Form.Select id="" disabled className="ticket_Addon">
                        <option value={ticketdataedit.poApprover.name}>
                          {ticketdataedit.poApprover}
                        </option>
                      </Form.Select>
                    </>
                    ...
                    <>
                      ...
                      <Form.Select id="" disabled>
                        <option value={ticketdataedit.poApprover}>
                          {ticketdataedit.poApprover}
                        </option>
                      </Form.Select>
                    </>
                  ...
                  <Row>
                    <div className="form-check">
                      <input
                        type="radio"
                        value="yes"
                        checked={ticketdataedit.isRelated === true}
                      ...
                      <input
                        type="radio"
                        value="no"
                        checked={ticketdataedit.isRelated === false}
                        className="grey-background"
                        readOnly
                      />
                      <label> NO</label>
