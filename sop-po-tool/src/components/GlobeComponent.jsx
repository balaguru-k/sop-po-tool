...
const GlobeComponent = ({ countryCounts = {} }) => {
  ...
  let isIndiaMode = false;
  let indiaStaticMapUri, indiaStrokeMapUri;
  ...
  const getCountryCode = (countryName) => {
    return Object.keys(countryCodeMap).find(
      (code) => countryCodeMap[code] === countryName
    );
  };

  const countryColors = Object.keys(countryCounts).reduce((acc, code) => {
    const countryName = countryCodeMap[code];
    if (countryName) {
      acc[countryName] = "#2E5AA0";
    }
    return acc;
  }, {});
            ...
            <div className="globe-legend">
              {getLegendItems().map((item) => (
                <div key={item.code} className="legend-item">
                  ...
                  <span className="legend-text">{item.country} - {item.count}</span>
                </div>
              ))}
            </div>
          )}
